...
| `/admin/users/$id` | Grant / revoke app access for a Kratos identity (admin-only) |
| `/healthz` | Liveness probe |

## Architecture

- Every admin call (Kratos admin, Hydra admin, Keto write) happens inside a
...
1. **Port-forward** from a live cluster:
   ...
   kubectl -n identity port-forward svc/keto-write    4467:80 &
   ```
